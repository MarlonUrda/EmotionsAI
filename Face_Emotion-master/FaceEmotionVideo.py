...
prototxtPath = r"C:\Users\mgur2\OneDrive\Documentos\Tarea-3\Face_Emotion-master\face_detector\deploy.prototxt"
weightsPath = r"C:\Users\mgur2\OneDrive\Documentos\Tarea-3\Face_Emotion-master\face_detector\res10_300x300_ssd_iter_140000.caffemodel"
print(prototxtPath)
faceNet = cv2.dnn.readNet(prototxtPath, weightsPath)

#direccion del modelo
modelo = r"C:\Users\mgur2\OneDrive\Documentos\Tarea-3\Face_Emotion-master\modelo\modelFEC.h5"

# Carga el detector de clasificación de emociones
emotionModel = load_model(modelo)
...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		(Xi, Yi, Xf, Yf) = box
		(Enojado,disgustado,Miedo,Feliz,Neutral,Triste,Sorprendido) = pred


		label = ''
		# Se agrega la probabilidad en el label de la imagen
		label = "{}: {:.0f}%".format(classes[np.argmax(pred)], max(Enojado,disgustado,Miedo,Feliz,Neutral,Triste,Sorprendido) * 100)

		cv2.rectangle(frame, (Xi, Yi-40), (Xf, Yi), (0,0,255), -1)
		cv2.putText(frame, label, (Xi+5, Yi-15),cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255,255,255), 2)
